...
import board1.BoardBean;
import board1.UtilMgr;
import mnt.DBConnectionMgr;

public class BoardMgr2 {

   private DBConnectionMgr pool;
   private static final String  SAVEFOLDER = "C:\\websys\\1712782\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp0\\wtpwebapps\\Lab12\\fileupload";
   ...
   public int getTotalCount(String keyField, String keyWord) {
      ...
      try {
         con = pool.getConnection();
         if (keyWord.equals("null") || keyWord.equals("")) {
            sql = "select count(num) from tblBoard2";
            pstmt = con.prepareStatement(sql);
         } else {
            sql = "select count(num) from  tblBoard2 where " + keyField + " like ? ";
            pstmt = con.prepareStatement(sql);
            pstmt.setString(1, "%" + keyWord + "%");
         }
         rs = pstmt.executeQuery();
   ...
   public void insertBoard(HttpServletRequest req) {
      ...
      try {
         con = pool.getConnection();
         sql = "select max(num) from tblBoard2";
         pstmt = con.prepareStatement(sql);
         rs = pstmt.executeQuery();
         int ref = 1;
         if (rs.next())
   ...
   public void updateBoard(BoardBean bean) {
      ...
      try {
         con = pool.getConnection();
         sql = "update tblBoard2 set name=?,subject=?,content=? where num=?";
         pstmt = con.prepareStatement(sql);
